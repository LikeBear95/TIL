...
def create_user(data):
    ...
    for i in data:
        # 반환값이 False 이면
        if is_validation(i)[0] == 'False':
            # 잘못된 데이터 탐색
            for j in i:
                # 잘못된 데이터를 찾아서
                if j in is_validation(i)[1]:
                    # 값을 False로 바꾼다
                    i[j] = 'None'
            user_list.append(i)
            error_count += 1
        if is_validation(i)[0] == 'blocked':
            error_count += 1
...
def is_validation(data):
    is_ok = []
    # 조건 b
    # 따로 if 조건을 걸어 'blocked'일 경우 검사 종료
    if data['company'] in black_list:
        is_ok.append('blocked')
    else:
        check_list = []
        # 조건 a
        ...
        # check_list 길이로 True or False 반환
        if len(check_list) == 0:
            is_ok.append('True')
        else:
            is_ok.append('False')
            is_ok.append(check_list)
    # ((bTF),[오류리스트]) 반환
    return is_ok
